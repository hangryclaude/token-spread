...
New units, as built (2026-08-31 — TDD workflow + adversarial review + post-review fixes).
No new importer was needed: the frozen `importAdminUsageReport` already speaks the poller's
input shape, so the poller feeds it directly.

```
src/pool/
├── types.ts      the pinned data contracts (this section's vocabulary)        [pure]
├── config.ts     member↔workspace↔key mapping, strict validation, defaults    [pure]
├── ledger.ts     append-only rows, provider-keyed idempotency, JSONL replay   [pure]
├── budget.ts     balances → thresholds → hard cap, integer basis points       [pure]
├── reserve.ts    blind-window holdback: peak observed burn × exposure window  [pure]
├── reconcile.ts  ledger vs cost_report, union compare, 1¢ + 0.1% tolerance    [pure]
├── poller.ts     the only I/O: fetch → raw-first → append → gate → (enforce)  [I/O shell]
└── cli.ts        status | poll | reconcile | credit; dry-run by default       [I/O shell]
```
