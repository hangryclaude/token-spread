 ...
 * ever reached a balance). rates.ts/pricing.ts fix 1 cent = 1_000_000 micro-cents
 * everywhere else in this codebase, so the whole-cent intPart scales by that same
 * 1_000_000. The fraction is padded to 6 digits — millionths of a cent, i.e.
 * micro-cents directly — which is this system's own precision floor, so anything
 * finer is noise, not signal. Anything not exactly `\d+(\.\d{1,6})?` is refused —
 * signs, exponents, sub-micro-cent fraction digits, stray characters all come back null.
 */
function parseDecimalCentsToMicroCents(amount: string): number | null {
  ...
  return Number(intPart) * 1_000_000 + Number(fracPadded);
}
...
export function parseCostReport(
  ...
  for (const page of pages) {
    for (const bucket of page?.data ?? []) {
      // A rotten bucket is counted and skipped, never thrown — one bad row in the
      // provider's report must not cancel the whole night's verdicts (spec §6).
