...
test("an adjustment row (e.g. a chargeback) folds into balance and spendable, not just credited-consumed", () => {
  // credited 2_000_000, consumed 0, adjustment -1_900_000 (chargeback) ->
  // balance = 2_000_000 - 0 + (-1_900_000) = 100_000; reserve 0 -> spendable 100_000.
  // Recomputing balance from credited-consumed alone (ignoring the adjustment) would
  // leave both fields at 2_000_000 and wrongly report the member as uncapped-and-flush.
  const d = budgetDecision({
    memberId: "m1",
    creditedMicroCents: 2_000_000,
    consumedMicroCents: 0,
    adjustmentMicroCents: -1_900_000,
    reserveMicroCents: 0,
    alreadyAlertedPcts: [],
    thresholds: [50, 80, 95],
  });
  expect(d.balanceMicroCents).toBe(100_000);
  expect(d.spendableMicroCents).toBe(100_000);
  expect(d.hardCap).toBe(false);
});
