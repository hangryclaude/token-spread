...
test("parses a single USD result into integer micro-cents", () => {
  // "123.45" cents -> intPart 123 * 10_000 + fracPart "4500" = 1_230_000 + 4_500 = 1_234_500
  const pages = [
  ...
  expect(rows).toEqual([{ day: "2026-08-01", workspaceId: "ws1", amountMicroCents: 1_234_500 }]);
...
test("a null workspace_id maps to the default workspace", () => {
  ...
  expect(rows).toEqual([{ day: "2026-08-01", workspaceId: "default", amountMicroCents: 10_000 }]);
});

test("a four-decimal-digit amount converts to the exact micro-cent", () => {
  // "0.0001" -> intPart 0 * 10_000 + fracPart "0001" = 1
  const pages = [
    { data: [{ starting_at: "2026-08-01T00:00:00Z", results: [
  ...
  const { rows, malformed } = parseCostReport(pages);
  expect(rows).toEqual([{ day: "2026-08-01", workspaceId: "ws1", amountMicroCents: 1 }]);
  expect(malformed).toBe(0);
});

test("exponents, extra fraction digits, signs, and non-USD currency are all malformed", () => {
  const bad = ["1e2", "12.34567", "-5"];
  const pages = [
    { data: [{ starting_at: "2026-08-01T00:00:00Z", results: [
...
test("multiple results for the same day and workspace sum", () => {
  ...
  const { rows } = parseCostReport(pages);
  // 1.00 -> 10_000; 2.50 -> 25_000; sum 35_000
  expect(rows).toEqual([{ day: "2026-08-01", workspaceId: "ws1", amountMicroCents: 35_000 }]);
});
